...
private class SpeechManager: NSObject, AVSpeechSynthesizerDelegate {
    ...
    func speak(_ text: String) {
        ...
        DispatchQueue.main.async {
            ...
            // AVSpeechSynthesizer en watchOS ignora utterances encoladas
            // inmediatamente después de stopSpeaking(); esperamos un ciclo
            // corto a que termine de cancelar antes de re-encolar.
...
struct ContentView: View {
    ...
    @State private var loadingLabel = "Pensando..."
    ...
    private let thinkingMessages = [
        "Pensando...", "Consultando a Rasputina...", "Buscando en la memoria...",
        "Conectando neuronas...", "Procesando...", "Casi listo...",
        "Analizando tu pregunta...", "Un momento..."
    ]

    @AppStorage("chat_history") private var historyJSON: String = "[]"
    ...
    private var recordedURL: URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("openclaw_recording.wav")
    }
    ...
    func checkHealth() async {
        guard let url = URL(string: "https://open.panicbots.com/watch/health") else { return }
    ...
    func transcribeAndChat() async {
        ...
        guard let transcribeURL = URL(string: "https://open.panicbots.com/watch/transcribe") else { return }
        ...
        do {
            ...
            guard let json = try JSONSerialization.jsonObject(with: transcribeData) as? [String: Any],
                  let text = json["text"] as? String, !text.isEmpty else {
                await MainActor.run {
                    stopLoadingCycle()
                    setAssistantReply("No se entendió el audio")
                    loading = false
                    showReply = true
                    isStreaming = false
    ...
    func sendMessage(text: String) async {
        ...
        guard let url = URL(string: "https://open.panicbots.com/watch/chat/stream") else {
        ...
        let body: [String: Any] = ["message": text, "session_key": "watch", "history": historyPayload]
        ...
        do {
            ...
            for try await line in bytes.lines {
                ...
                if json["done"] as? Bool == true {
                    finalReply = (json["reply"] as? String) ?? fullReply
                    break
                }
            }
    ...
    func sendMessageClassic(text: String) async {
        guard let url = URL(string: "https://open.panicbots.com/watch/chat") else {
            await MainActor.run { reply = "URL inválida"; loading = false; showReply = true }
            return
        }
        ...
        do {
            ...
            if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
               let agentReply = json["reply"] as? String {
                await MainActor.run {
                    stopLoadingCycle()
                    loading = false
                    showReply = true
                ...
                await MainActor.run {
                    stopLoadingCycle()
                    isStreaming = false
                    setAssistantReply("Respuesta inválida")
                    loading = false
                    showReply = true
                    speak("Respuesta inválida")
                }
            }
        } catch {
            if Task.isCancelled || error is CancellationError || (error as? URLError)?.code == .cancelled {
    ...
    func resetBackendSession() async {
        guard let url = URL(string: "https://open.panicbots.com/watch/reset") else { return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.httpBody = try? JSONSerialization.data(withJSONObject: ["session_key": "watch"])
